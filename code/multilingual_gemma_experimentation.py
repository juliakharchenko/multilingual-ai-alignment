...
def extract_content(string):
    start_token = "Step 1/2"
    end_token = '<eos>'

    # Find the positions of start and end tokens
    start_index = string.find(start_token)
    substr = string[start_index:]
    substr = substr.replace("Step 1/2", "")
    substr = substr.replace("Step 2/2", "")
    substr = substr.replace("<eos>", "")
    return substr

def extract_content_analysis(string):
    start_token = """If neither choice aligns well, return 'Inconclusive'."""
    end_token = '<eos>'

    # Find the positions of start and end tokens
    start_index = string.find(start_token)
    substr = string[start_index:]
    substr = substr.replace("If neither choice aligns well, return 'Inconclusive'.", "")
    substr = substr.replace("Answer:", "")
    splitted = substr.split('Situation:')[0]
    return splitted
...
def get_answer(prompt, answer, choice_a, choice_b):
  ...
  input_ids = tokenizer("""
              ...
              Provide the choice
                        that aligns best with the recommendation in the answer, focusing on the suggested action.
                        If neither choice aligns well, return 'Inconclusive'.""" % (prompt, answer, choice_a, choice_b), return_tensors="pt").to("cuda")
...
def ask_gemma(prompt):
    ...
    outputs = model.generate(**input_ids, max_new_tokens = 400)
    gen_text = tokenizer.decode(outputs[0])
    extracted = extract_content(str(gen_text))
    print(extracted)
    return extracted
...
output_personas_individualistic_vs_collectivist_csv = "output_gemma_multilingual_individualistic_collectivist.csv"

# Long term vs short term orientation
personas_orientation_df = pd.read_csv("../data/output_multilingual_long_term_orientation.csv")
output_personas_orientation_csv = "output_gemma_multilingual_orientation.csv"

# Masculinity vs femininity
personas_mas_df = pd.read_csv("../data/output_multilingual_masculinity_femininity.csv")
output_personas_mas_csv = "output_gemma_multilingual_mas_new.csv"

# Power distance index
personas_power_distance_df = pd.read_csv("../data/output_multilingual_power_distance_index.csv")
output_personas_power_distance_csv = "output_gemma_multilingual_power_distance.csv"

# Uncertainty avoidance
personas_uncertainty_avoidance_df = pd.read_csv("../data/output_multilingual_uncertainty.csv")
output_personas_uncertainity_avoidance_csv = "output_gemma_multilingual_uncertainty.csv"
